...
async function test() {
  ...
  batch.add(web3.eth.getBalance.request(addr, 'latest'));
  batch.add(lrcToken.methods.balanceOf(addr).call.request({from: addr}));
  const res = await batch.execute();
  console.log("res:", res);

  // const balance = await lrcToken.methods.balanceOf(addr).call();
  // console.log(balance);
}
